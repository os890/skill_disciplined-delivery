...
**Too much and too little are measured against the same yardstick.** More than the definitions ask for is scope creep; less than they require is an unfinished change — the acceptance criteria, the NFRs and the phase 2 edges say which, so it is a question with an answer rather than an opinion. Under-delivery is the more expensive of the two, because scope creep is visible in the diff while a case nobody handled looks exactly like a case nobody needed.
