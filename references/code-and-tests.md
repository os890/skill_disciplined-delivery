...
- **Formatted by the language's default formatter** — `gofmt`, `rustfmt`, `dotnet format`, Prettier, Black/Ruff format, Spotless with the project's chosen style. No second formatter on top, no hand-tuned alignment, no mixed styles, no reformatting untouched code. The formatter check runs in the build.
...
- **No premature minimalism either.** The smallest edit that makes the happy path run is not the smallest *correct* edit. Before calling a change done, walk the edge cases named in phase 2 — empty, missing, duplicate, concurrent, oversized, unauthorized, downstream failure — and the paths the change touches without obviously altering: other callers, persistence, configuration, migration, observability. A shortcut taken to reach the goal faster does not make the work smaller, it moves the cost to whoever meets the gap.
...
**Under-delivery is the more expensive of the two failures `SKILL.md` phase 5 names**: scope creep is visible in the diff, while a case nobody handled looks exactly like a case nobody needed.

And **speed is never a reason to bypass a project rule** — an existing abstraction, a gate, an ADR, a convention in `CLAUDE.md`. A rule that genuinely obstructs the work is a pushback (`SKILL.md` § Pushback), never a silent exception.
...
- **Embedded first**: in-process test framework, embedded server, embedded broker or database where a credible one exists.
  ...
  - **A failing embedded test can mean the framework does not support that feature**, not that the code is wrong. Its documentation and its own scenario tests enumerate what is supposed to work — check those before changing production code or wrapping the call in a retry, a flush or a widened timeout. "Unsupported" is a claim that needs that evidence, and it **moves** the coverage rather than dropping it: the part the framework cannot reach gets an end-to-end test against a real runtime instead, with the reason named at the test.
- **Containers when embedded won't do** — the app needs a real runtime, or a real database, message broker or cache, or the in-process framework does not support what the test has to exercise. **Podman** by default (Docker Engine/CLI if the project already requires it — not Docker Desktop, which is paid above a company-size threshold); drive it from a framework such as Testcontainers so the suite owns the lifecycle and nothing leaks between runs.
