...
- **Where the work-in-progress convention is on** (`ADR-0001`), every subject opens with a status token, so the state of a commit is readable without running anything:
