...
- If the project uses the work-in-progress convention (`ADR-0001`): does every subject carry a status token, and is **no pushed commit still `UNTESTED`**? One that is means either the promotion to `WORKING` was skipped or the work genuinely has no test — find out which.
