...
**Both of those are measured against the same yardstick, and neither is a matter of taste.** More than the definitions ask for is scope creep; less than they require is an unfinished change. The acceptance criteria, the NFRs and the phase 2 edge catalogue say which one you are looking at — so "is this too much or too little?" is a question with an answer, not an opinion.

Of the two, under-delivery is the more expensive: scope creep is visible in the diff and gets caught in review, while a case nobody handled looks exactly like a case nobody needed. And **speed is never a reason to bypass a project rule** — an existing abstraction, a gate, an ADR, a convention in `CLAUDE.md`. If a rule genuinely obstructs the work, that is a pushback (`SKILL.md` § Pushback), never a silent exception.
